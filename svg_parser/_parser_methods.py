    ...
    for element in list(root):
        ...
        if draw_hidden or visible:
            if element.tag == "{%s}path" % NAMESPACES["svg"]:
                path = Path(
                    element.attrib["d"], canvas_height, transform_origin, transformation
                )
...
def parse_string(
    svg_string: str, transform_origin=True, canvas_height=None, draw_hidden=False
) -> List[Curve]:
    """
...
def parse_file(
    file_path: str, transform_origin=True, canvas_height=None, draw_hidden=False
) -> List[Curve]:
    """
