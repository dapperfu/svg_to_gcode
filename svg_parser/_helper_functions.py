...
def _has_style(element: ElementTree.Element, key: str, value: str) -> bool:
    ...
    """
    return element.get(key) == value or (
        element.get("style") and f"{key}:{value}" in element.get("style")
    )
...
def parse_root(
    ...
    if canvas_height is None:
        ...
        canvas_height = (
            float(height_str) if height_str.isnumeric() else float(height_str[:-2])
        )
        ...
        for element in list(root):
            ...
            if not transparent and visible:
                if element.tag == "{%s}path" % NAMESPACES["svg"]:
                    path = Path(
                        element.attrib["d"], canvas_height, transform, transform
                    )
                    curves.extend(path.curves)
...
def parse_string(
    svg_string: str, canvas_height=None, transform=True, draw_hidden=False
) -> List[Curve]:
    """
...
def parse_file(
    file_path: str, canvas_height=None, transform=True, draw_hidden=False
) -> List[Curve]:
    """
