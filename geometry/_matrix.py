...
class Matrix:
    ...
    def __init__(self, matrix_list):
        ...
        if not all([len(row) == self.number_of_columns for row in matrix_list]):
            raise ValueError(
                "Not a matrix. Rows in matrix_list have different lengths."
            )
    ...
    def multiply_matrix(self, other_matrix: "Matrix"):
        if self.number_of_columns != other_matrix.number_of_rows:
            raise ValueError(
                f"can't multiply matrices. The first matrix must have the same number of columns as the "
