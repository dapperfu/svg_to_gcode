...
class SmoothArcChain(Chain):
    ...
    def append(self, arc2: CircularArc):
        if self._curves:
            ...
            try:
                ...
                raise ValueError(
                    f"The end of the last arc is different from the start of the new arc, "
                    f"|{arc1.end} - {arc2.start}| >= {TOLERANCES['input']}"
                )

            try:
                assert (
                    abs(arc1.derivative(arc1.end) - arc2.derivative(arc2.start))
                    < TOLERANCES["input"]
                )
            except AssertionError:
                raise ValueError(
                    f"The last arc and the new arc form a discontinues curve, "
    ...
    def cubic_bazier_to_arcs(bazier, _arcs=None):
        ...
        incenter_point = (
            start_length * end + end_length * start + base_length * tangent_intersect
